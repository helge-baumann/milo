...
EVS_defl <- EVS %>% 
  mutate(
    ...
    # Deflationierung
    lohn_summe_winsor = if_else(welle == "2013", lohn_summe_winsor * 1/0.985, lohn_summe_winsor*1/1.038),
...
# Auswertung nach Kriterien
 EVS_defl %>% pivot_longer(cols=c(gesamt, lohn_haushalt, milo_hh_stat, milo_hh_dyn, hh_size, ET_hh), 
  ...
  summarise(
    ...
    `Lohnsumme gesamt`= round(wtd.mean(lohn_summe_winsor/3, gewicht), digits=1),
    `Lohnsumme Anteil am Bruttohaushaltseinkommen`= round(wtd.mean(lohn_anteil_winsor/3, gewicht), digits=1),
