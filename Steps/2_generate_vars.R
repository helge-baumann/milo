...
for (i in 1:6) { # für alle Personen im Haushalt, 1-6
    ...
    mutate(
      ...
      "lohn_{i}" :=
        if_else(
          ...
          rowSums(.[c(paste0("EF", c(110, 113:114, 120:121, 128), "U", i), 
                      paste0("basislohn_", i))]), NA_real_
        ),
      # Bruttomonatslohn
      ...
      "az_{i}" := case_when(get(paste0("EF", 7 + i, "U16")) > 0 ~ 
  ...
  attributes(evs13[[paste0("sex_", i)]])$label <-
    paste0("Person ", i, ": Geschlecht")
     attributes(evs13[[paste0("vollzeit_", i)]])$label <-
    paste0("Person ", i, ": Teilzeit (0) / Vollzeit (1)")
  ...
  evs18 <- evs18 %>%
    ...
    mutate(
      ...
      # Vereinbarte Wochenarbeitszeit (0 = trifft nicht zu / keine Angabe)
      "az_{i}" := case_when(get(paste0("EF", 7 + i, "U17")) > 0 ~ 
